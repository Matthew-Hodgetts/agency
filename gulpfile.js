...
Theme Name
-------------------------------------------------------------------------------------------------- */
const themeName = 'panther_mpolar';
...
-------------------------------------------------------------------------------------------------- */
const headerJS = [];
const footerJS = ['node_modules/jquery/dist/jquery.js', 'src/js/**'];
...
gulp.task('download-wordpress', () => {
				remoteSrc(['latest.zip'], {base: 'https://wordpress.org/'}).pipe(gulp.dest('build/'));
});
...
gulp.task('build-dev', [
				...
				connect.server({
								...
								browserSync({proxy: '127.0.0.1:3020'});
				});
});
...
gulp.task('reload-js', [
				'footer-scripts-dev', 'header-scripts-dev'
], (done) => {
				browserSync.reload();
				done();
});
...
const devServerReady = 'Your development server is ready, start the workflow with the command: $ \x1b[1m' +
								'npm run dev\x1b[0m';
const buildNotFound = errorMsg + ' ⚠️　- You need to install WordPress first. Run the command: $ \x1b[1mnpm run ins' +
								'tall:wordpress\x1b[0m';
